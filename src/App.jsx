...
// Importación de las páginas modulares
...
export default function App() {
    ...
    const triggerMainReport = (action) => {
        ...
        let titleMode = mode === 'pay' ? (payTab === 'realizados' ? 'Pagos Realizados' : 'Pagos Pendientes') : (mode === 'collect' ? 'Cobros' : (mode === 'wallet' ? 'Cartera Cheques' : (mode === 'expenses' ? 'Gastos Mensuales' : 'Balance')));
        ...
        if (mode === 'balance') {
            ...
            balanceData.dailyAgenda.forEach(day => { 
                bodyHTML += `<tr><td>${new Date(day.date + 'T00:00:00').toLocaleDateString('es-AR')}</td><td class="amount green">${day.inflow > 0 ? formatMoney(day.inflow) : '-'}</td><td class="amount" style="color:blue">${day.outflow > 0 ? formatMoney(day.outflow) : '-'}</td><td class="amount">${formatMoney(day.net)}</td></tr>`; 
            });
            bodyHTML += `</tbody></table>`;
        } else {
            let modeTotal = mode === 'pay' ? (payTab === 'realizados' ? payItems.filter(i=>i.status==='paid').reduce((acc,i)=>acc+(i.paidAmount || i.amount),0) : balanceData.pay) : (mode === 'wallet' ? balanceData.wallet : (mode === 'expenses' ? balanceData.expenses : balanceData.collect));
            bodyHTML += `<div class="total-box">Total ${(mode === 'pay' && payTab === 'realizados') ? 'Cancelado' : 'Pendiente'}: ${formatMoney(modeTotal)}</div><table><thead><tr><th style="width:30%">Referencia</th><th style="width:15%">Vencimiento</th><th style="text-align:right;width:15%">Total Original</th><th style="text-align:right;width:20%">${(mode === 'pay' && payTab === 'realizados') ? 'Abonado' : 'Pendiente'}</th><th style="text-align:right;width:20%">Acumulado</th></tr></thead><tbody>`;
            let runningAccumulator = 0;
            let groupsToProcess = groupedView.type === 'flat' ? [{ key: 'General', label: 'Listado General', total: modeTotal, items: groupedView.data }] : groupedView.data;
            groupsToProcess.forEach(group => {
                const isRealizados = (mode === 'pay' && payTab === 'realizados');
                const sortedItems = [...group.items].filter(item => isRealizados ? item.status === 'paid' : item.status !== 'paid').sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
                if (sortedItems.length === 0) return;
                let groupTitle = groupedView.type === 'client-grouped' || groupedView.type === 'month-grouped' || groupedView.type === 'flat' ? group.label || 'Listado General' : new Date(group.key + 'T00:00:00').toLocaleDateString('es-AR');
    ...
    const triggerCalendarReport = (action) => {
        ...
        if (Object.keys(bTotals).length > 0) {
            bodyHTML += `<div style="margin-bottom:20px; padding:10px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:4px;"><h3 style="margin-top:0; font-size:12px; color:#334155; border-bottom:1px solid #cbd5e1; padding-bottom:4px;">Totales de Pagos Pendientes por Banco (Este mes)</h3><ul style="margin:0; padding-left:20px; font-size:11px; font-family:monospace;">`;
            Object.entries(bTotals).sort((a,b)=>b[1]-a[1]).forEach(([bank, total]) => { bodyHTML += `<li><strong>${bank}:</strong> ${formatMoney(total)}</li>`; });
            bodyHTML += `</ul></div>`;
        }
        bodyHTML += `<table><thead><tr><th class="date-col">Fecha</th><th>Ingresos (Pendientes)</th><th>Egresos (Pendientes)</th><th>Acumulado</th></tr></thead><tbody>`;
        days.forEach(day => {
            if(!day) return;
            let inflowHTML = ""; if(day.inflowItems.length > 0) { const pendingInflows = day.inflowItems.filter(i => i.status !== 'paid'); if (pendingInflows.length > 0) { pendingInflows.forEach(i => { const ref = i.number ? `Ref: ${i.number}` : ''; const remaining = i.amount - (i.paidAmount || 0); inflowHTML += `<div style="margin-bottom:6px;"><div style="font-weight:bold; color:#16a34a;">${formatMoney(remaining)}</div><div style="font-size:9px; color:#555;">(${i.payee} ${ref ? '- '+ref : ''})</div></div>`; }); } else { inflowHTML = "-"; } } else { inflowHTML = "-"; }
            let outflowHTML = ""; if(day.outflowItems.length > 0) { const pendingOutflows = day.outflowItems.filter(i => i.status !== 'paid'); if (pendingOutflows.length > 0) { pendingOutflows.forEach(i => { const ref = i.number ? `Ref: ${i.number}` : ''; const bankTag = i.bank ? ` 🏦 ${i.bank}` : ''; const remaining = i.amount - (i.paidAmount || 0); outflowHTML += `<div style="margin-bottom:6px;"><div style="font-weight:bold; color:#dc2626;">${formatMoney(remaining)}</div><div style="font-size:9px; color:#555;">(${i.payee} ${ref ? '- '+ref : ''}${bankTag})</div></div>`; }); } else { outflowHTML = "-"; } } else { outflowHTML = "-"; }
    ...
    const triggerRangeReport = (action) => {
        ...
        const combined = [...mappedPay, ...mappedExp, ...mappedCol, ...mappedWal].filter(i => i.status !== 'paid' && i.dueDate && i.dueDate >= startDate && i.dueDate <= endDate).sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
        ...
        let bodyHTML = `<h1>Reporte de Movimientos Pendientes por Fecha</h1><p class="meta">Período: ${new Date(startDate+'T00:00:00').toLocaleDateString('es-AR')} al ${new Date(endDate+'T00:00:00').toLocaleDateString('es-AR')} • Generado: ${new Date().toLocaleDateString('es-AR')}</p>`;
        if (Object.keys(bTotals).length > 0) {
            bodyHTML += `<div style="margin-bottom:20px; padding:10px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:4px;"><h3 style="margin-top:0; font-size:12px; color:#334155; border-bottom:1px solid #cbd5e1; padding-bottom:4px;">Totales de Pagos Pendientes por Banco (En este rango)</h3><ul style="margin:0; padding-left:20px; font-size:11px; font-family:monospace;">`;
            Object.entries(bTotals).sort((a,b)=>b[1]-a[1]).forEach(([bank, total]) => { bodyHTML += `<li><strong>${bank}:</strong> ${formatMoney(total)}</li>`; });
            bodyHTML += `</ul></div>`;
        }
        bodyHTML += `<table><thead><tr><th style="width:15%">Fecha</th><th style="width:45%">Concepto / Cliente</th><th style="width:10%">Tipo</th><th style="text-align:right; width:15%">Ingreso (Pendiente)</th><th style="text-align:right; width:15%">Egreso (Pendiente)</th></tr></thead><tbody>`;
        combined.forEach(item => {
            const pendingAmount = item.amount - (item.paidAmount || 0);
            if (item.isPositive) totalPos += pendingAmount; else totalNeg += pendingAmount;
            const bankTag = (!item.isPositive && item.bank) ? `<br/><span style="color:#0284c7;font-size:9px;font-weight:bold;">🏦 ${item.bank}</span>` : '';
            bodyHTML += `<tr><td class="date">${new Date(item.dueDate+'T00:00:00').toLocaleDateString('es-AR')}</td><td><strong>${item.payee}</strong> ${item.number ? `<br/><span style="color:#666;font-size:9px">Ref: ${item.number}</span>` : ''}${bankTag}</td><td style="font-size:10px">${item.reportType}</td><td class="amount green">${item.isPositive ? formatMoney(pendingAmount) : '-'}</td><td class="amount red">${!item.isPositive ? formatMoney(pendingAmount) : '-'}</td></tr>`;
        });
        const balance = totalPos - totalNeg; const balanceColor = balance >= 0 ? 'green' : 'red';
        bodyHTML += `</tbody></table><div style="margin-top:20px; display:flex; justify-content:space-between; gap:10px; page-break-inside: avoid;"><div style="flex:1; padding:15px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; text-align:center;"><span style="display:block; font-size:10px; color:#166534; text-transform:uppercase; font-weight:bold; margin-bottom:5px;">Total Ingresos (Pendientes)</span><span style="font-size:16px; font-weight:900; color:#15803d;">${formatMoney(totalPos)}</span></div><div style="flex:1; padding:15px; background:#fef2f2; border:1px solid #fecaca; border-radius:8px; text-align:center;"><span style="display:block; font-size:10px; color:#991b1b; text-transform:uppercase; font-weight:bold; margin-bottom:5px;">Total Egresos (Pendientes)</span><span style="font-size:16px; font-weight:900; color:#b91c1c;">${formatMoney(totalNeg)}</span></div><div style="flex:1; padding:15px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; text-align:center;"><span style="display:block; font-size:10px; color:#334155; text-transform:uppercase; font-weight:bold; margin-bottom:5px;">Balance Neto Proyectado</span><span style="font-size:18px; font-weight:900;" class="${balanceColor}">${balance >= 0 ? '+' : ''}${formatMoney(balance)}</span></div></div>`;

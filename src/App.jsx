...
// Importación de las páginas que creamos en el paso anterior
...
export default function App() {
    ...
    const groupedView = useMemo(() => {
        ...
        if (mode === 'cashbox' || mode === 'pallets') return null;
    ...
    const triggerCalendarReport = (action) => {
        ...
        // Extraer resumen de bancos del mes actual (SOLO PENDIENTES)
        const year = currentCalDate.getFullYear(); 
        const month = currentCalDate.getMonth();
        const firstDateStr = `${year}-${String(month + 1).padStart(2, '0')}-01`;
    ...
    const handleAdd = async (e) => {
        ...
        const itemData = { 
            ...
            subtype: newItem.subtype || 'cheque', 
        ...
        if (editingId) { 
            await updateDoc(doc(db, targetCollection, 'main_list', 'items', editingId), { 
                ...itemData, 
            ...
            if (mode === 'expenses' && newItem.isRecurring && newItem.installments > 1) {
                ...
                for (let i = 0; i < newItem.installments; i++) {
                    ...
                    if (newItem.dueDate) { 
                        const d = new Date(newItem.dueDate + 'T00:00:00'); 
    ...
    const confirmPayment = async () => {
        ...
        let currentPaid = item.paidAmount || 0;
        let paidNow = 0;
        let newStatus = 'pending';

        if (paymentModal.type === 'total') { 
            paidNow = item.amount - currentPaid;
            currentPaid = item.amount; 
            newStatus = 'paid'; 
        } else { 
            paidNow = parseFloat(paymentModal.amount); 
            if (!isNaN(paidNow)) { 
                currentPaid += paidNow; 
                if (currentPaid >= item.amount) { 
                    currentPaid = item.amount; 
                    newStatus = 'paid'; 
                } else { 
                    newStatus = 'partial'; 
        ...
        <div className="pb-24 max-w-4xl mx-auto min-h-screen bg-gray-50 shadow-2xl relative transition-colors duration-500">
            <div className={`${theme.bg} text-white p-4 sticky top-0 z-30 shadow-lg transition-colors duration-500`}>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-1 items-center">
                        {deferredPrompt && ( 
                            <button 
                                onClick={installApp} 
                                className="bg-white/20 hover:bg-white/30 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 mr-1"
                            >
                        ...
                        {mode !== 'cashbox' && mode !== 'pallets' && (
                            <>
                                <button 
                                    ...
                                    className="p-2 relative bg-white/20 rounded-full hover:bg-white/30" 
                ...
                <div className="bg-black/20 p-1 rounded-xl flex text-[10px] sm:text-xs overflow-x-auto scrollbar-hide">
                    <button 
                        onClick={() => changeMode('cashbox')} 
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'cashbox' ? 'bg-white text-cyan-800 shadow-md' : 'text-white/60'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'expenses' ? 'bg-white text-orange-800 shadow-md' : 'text-white/60'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'pay' ? 'bg-white text-blue-800 shadow-md' : 'text-white/60'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'balance' ? 'bg-white text-violet-800 shadow-md' : 'text-white/60'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'collect' ? 'bg-white text-emerald-800 shadow-md' : 'text-white/60'}`}
                    >
                        <IconBriefcase className="w-4 h-4"/> 
                        <span>COBROS</span>
                    </button>
                    <button 
                        onClick={() => changeMode('wallet')} 
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'wallet' ? 'bg-white text-indigo-800 shadow-md' : 'text-white/60'}`}
                    >
                        <IconFolder className="w-4 h-4"/> 
                        <span>CARTERA</span>
                    </button>
                    <button 
                        onClick={() => changeMode('pallets')} 
                        className={`flex-1 py-2 px-3 rounded-lg font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all whitespace-nowrap ${mode === 'pallets' ? 'bg-white text-teal-800 shadow-md' : 'text-white/60'}`}
                    >
                ...
                <div className="fixed inset-0 modal-overlay z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-fade-in">
                        <h3 className="text-lg font-bold text-gray-800 mb-4">
                            Confirmar {paymentModal.item.type === 'collect' ? 'Cobro' : 'Pago'}
                        </h3>
                            ...
                            {paymentModal.type === 'partial' && (
                                <div>
                                    <label className="text-xs text-gray-500 block mb-1">
                                        Monto a {paymentModal.item.type === 'collect' ? 'Cobrar' : 'Pagar'} Ahora
                                    </label>
                                    <input 
                        ...
                        <div className="mt-6 flex gap-2">
                            <button 
                                onClick={() => setPaymentModal({isOpen:false, item:null, method:'transfer', proof:'', type: 'total', amount: ''})} 
                                className="flex-1 py-2 border rounded-lg text-gray-500 hover:bg-gray-50 font-bold transition-colors"
                            >
                                Cancelar
                            </button>
                            <button 
                ...
                <div className="fixed inset-0 modal-overlay z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md animate-fade-in relative">
                        ...
                        <h3 className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                            <IconEye className="text-purple-500"/> Historial de Pagos
                        </h3>
                        <p className="text-sm font-bold text-gray-600 mb-4">
                            {historyModal.item.payee} 
                            {historyModal.item.number && <span className="font-mono text-gray-400 ml-2">#{historyModal.item.number}</span>}
                        </p>
                        
                        <div className="bg-gray-50 rounded-xl p-4 mb-4 flex justify-between text-sm shadow-inner border border-gray-200">
                            <div>
                                <span className="block text-[10px] uppercase text-gray-500">Total Original</span>
                                <span className="font-bold">{formatMoney(historyModal.item.amount)}</span>
                            </div>
                            <div>
                                <span className="block text-[10px] uppercase text-gray-500">Abonado</span>
                                <span className="font-bold text-green-600">{formatMoney(historyModal.item.paidAmount || 0)}</span>
                            </div>
                            <div className="text-right">
                                <span className="block text-[10px] uppercase text-gray-500">Saldo Pendiente</span>
                                <span className="font-bold text-red-500">{formatMoney(historyModal.item.amount - (historyModal.item.paidAmount || 0))}</span>
                            </div>
                        </div>
                                    ...
                                    <div key={i} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg bg-white shadow-sm hover:shadow transition-shadow">
                                        <div>
                                            <p className="text-xs font-bold text-gray-800">
                                                ...
                                                <span className="text-gray-400 font-normal ml-1">• {h.method.toUpperCase()}</span>
                                            </p>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <IconFileText className="text-blue-500 w-5 h-5"/> Reporte por Fechas
                            </h3>
                            <button 
                                onClick={() => setRangeReportModal({...rangeReportModal, isOpen: false})} 
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input 
                                    type="date" 
                                    className="w-full p-3 bg-gray-50 border rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" 
                                    value={rangeReportModal.startDate} 
                                    onChange={e => setRangeReportModal({...rangeReportModal, startDate: e.target.value})} 
                                />
                            </div>
                            <div>
                                ...
                                <input 
                                    type="date" 
                                    className="w-full p-3 bg-gray-50 border rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" 
                                    value={rangeReportModal.endDate} 
                                    onChange={e => setRangeReportModal({...rangeReportModal, endDate: e.target.value})} 
                                />
                            </div>
                            <div className="flex gap-2 mt-4 pt-2 border-t">
                                <button 
                                    onClick={() => {
                                        triggerRangeReport('download'); 
                                        setRangeReportModal({...rangeReportModal, isOpen: false});
                                    }} 
                                    className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-xl font-bold flex justify-center items-center gap-2 transition-colors"
                                >
                                    <IconDownload className="w-4 h-4"/> PDF
                                </button>
                                <button 
                                    onClick={() => {
                                        triggerRangeReport('print'); 
                                        setRangeReportModal({...rangeReportModal, isOpen: false});
                                    }} 
                    ...
                    <div className="bg-white w-full h-full max-w-4xl rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
                        <div className="bg-violet-600 text-white p-4 flex justify-between items-center shadow-md z-10">
                            <h2 className="font-bold text-lg flex items-center gap-2">
                                <IconCalendar className="w-6 h-6"/> Calendario Financiero
                            </h2>
                            <div className="flex gap-2">
                                <button 
                                    onClick={() => triggerCalendarReport('print')} 
                                ...
                                <button 
                                    onClick={() => triggerCalendarReport('download')} 
                                    className="p-2 bg-white/20 rounded-full hover:bg-white/30 transition-colors" 
                                    title="Descargar Calendario PDF"
                                >
                                    <IconDownload className="w-5 h-5"/>
                                </button>
                        ...
                        <div className="flex justify-between items-center p-4 bg-gray-50 border-b">
                            <button onClick={() => changeMonth(-1)} className="p-2 rounded hover:bg-gray-200">
                            ...
                            <div className="calendar-grid">
                                {generateCalendarDays().map((day, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`text-right font-bold ${isToday ? 'text-violet-700' : 'text-gray-400'} mb-1`}>
                                                {day.day}
                                            </div>
                        ...
                        <div className="p-3 bg-white border-t flex justify-end">
                             <button 
                                 onClick={() => setShowCalendar(false)} 
                                 className="bg-gray-200 text-gray-700 px-6 py-2 rounded-xl font-bold hover:bg-gray-300 transition-colors"
                             >
                                 Volver
                             </button>
                        </div>
